...
const NewsCard: React.FC<PreviewNews> = ({ title, pubDate, categories }) => {
  ...
  return (<div className="flex flex-col bg-white p-6  justify-between items-start gap-3 ">
    <div className="text-cyan-800 flex flex-row gap-2 border-2 rounded-lg p-0.5 px-1 border-solid border-cyan-300">{categories.map((categore,i)=>  (<p key={i}>{categore}</p>) )}</div>
